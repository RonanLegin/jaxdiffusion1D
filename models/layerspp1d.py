...
from . import layers1d as layers
...
class Upsample(nn.Module):
  ...
  def __call__(self, x):
    ...
    if not self.fir:
      h = jax.image.resize(x, (x.shape[0], H * 2, C), 'nearest')
      if self.with_conv:
        h = conv3x3(h, out_ch)
    else:
      if not self.with_conv:
        ...
        h = up_or_down_sampling.Conv1d(out_ch,
                                       kernel=3,
                                       up=True,
