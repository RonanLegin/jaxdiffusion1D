...
class AncestralSamplingPredictor(Predictor):
  ...
  def vesde_update_fn(self, rng, x, t, cond):
    sde = self.sde
  ...
  def vpsde_update_fn(self, rng, x, t, cond):
    sde = self.sde
    timestep = (t * (sde.N - 1) / sde.T).astype(jnp.int32)
...
class AnnealedLangevinDynamics(Corrector):
  ...
  def update_fn(self, rng, x, t, cond):
    sde = self.sde
    score_fn = self.score_fn
    n_steps = self.n_steps
    target_snr = self.snr
...
def shared_predictor_update_fn(rng, state, x, t, cond, sde, model, predictor, probability_flow, continuous):
  ...
  return predictor_obj.update_fn(rng, x, t, cond)


def shared_corrector_update_fn(rng, state, x, t, cond, sde, model, corrector, continuous, snr, n_steps):
  ...
  return corrector_obj.update_fn(rng, x, t, cond)
...
def get_ode_sampler(sde, model, shape, inverse_scaler,
  ...
  def ode_sampler(prng, pstate, z=None):
    ...
    if denoise:
      ...
      x = denoise_update_fn(step_rng, pstate, x, cond)
